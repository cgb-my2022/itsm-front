<template>
  <a-modal
    ...
    <service-node-info-model ref="serviceNodeInfoModel" ></service-node-info-model>
  </a-modal>
</template>
...
<script>
  ...
  export default {
    name: 'ServiceProcessInstPicModal',
    components: { ServiceNodeInfoModel },
    data () {
      return {
        title: '操作',
        visible: false,
    ...
    methods: {
      ...
      showNodeInfo(data,taskId){
        this.$refs.serviceNodeInfoModel.close();
        this.$refs.serviceNodeInfoModel.showInfo(data,taskId);
      },
    }
  }
</script>
