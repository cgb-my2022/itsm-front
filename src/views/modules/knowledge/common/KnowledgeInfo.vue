    ...
    <a-spin :spinning="confirmLoading">
      ...
      <a-form :form="form">
        <div class="title_info">
          <h2>知识信息</h2>
          <a-tag color="#87d068">{{ (rowInfo.status === 0 || !rowInfo.status) ? '草稿' : '修改中'}}</a-tag>
        </div>
        <a-row>
          <a-col :span="24">
        ...
        <div v-if="rowInfo.status && rowInfo.status != 0" class="title_info">
          <h2>修改说明</h2>
        </div>
        <a-row v-if="rowInfo.status && rowInfo.status != 0">
          <a-col :span="12">
            <a-form-item label="修改原因" :labelCol="labelCol1" :wrapperCol="wrapperCol">
              <a-input
