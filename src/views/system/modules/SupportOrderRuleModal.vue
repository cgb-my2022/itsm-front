    ...
    data () {
      return {
        ...
        url: {
          ...
          saveRule: '/system/usrServiceOrderRule/configSupportRule' // 保存接单规则
        },
        identity: '1',
        fileList: [],
        validatorRules: {
...
//                console.log(res.result);
      ...
      loadRules() {
        ...
        getAction(that.url.usrRule, { usrId: that.userId }).then((res) => {
          that.checkedDepartKeys =  res.result.supportRuleDepart.split(',');
          that.checkedDepartNameString = res.result.supportRuleDepartNames;
          that.busTypes = res.result.supportRuleBustype.split(',');
          if (res.result.supportRuleStatus === 2) {
      ...
      handleSubmit () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            var params = 'busTypes=' + this.busTypes.join(',') + '&deptIds=' + this.checkedDepartKeys.join(',') + '&departNames=' + this.checkedDepartNameString + '&usrId=' + this.userId + '&ruleStatus=' + this.ruleStatus;
